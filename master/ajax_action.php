...
if(isset($_POST['page']))
{
	...
	if($_POST['page'] == 'exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			foreach($result as $row)
			{
				...
				// $examTime=date('d.m.Y',strtotime($row['online_exam_datetime']));

				$sub_array[] = date('d.m.Y',strtotime($row['online_exam_datetime']));
	...
	if($_POST['page'] == 'question')
	{
		...
		if($_POST['action'] == 'fetch')
		{
			...
			$exam->query = "SELECT * FROM question_table WHERE online_exam_id = '".$exam_id."' 	AND ( ";
			...
			$exam->query = "SELECT * FROM question_table WHERE online_exam_id = '".$exam_id."'";
			...
			foreach($result as $row)
			{
				...
				{
					$edit_button = '<button type="button" name="edit" class="btn btn-primary btn-sm edit" 
					id="'.$row['question_id'].'">Edit</button>';

					$delete_button = '<button type="button" name="delete" class="btn btn-danger btn-sm delete" 
					id="'.$row['question_id'].'">Delete</button>';
				}
		...
		if($_POST['action'] == 'edit_fetch')
		{
			...
			foreach($result as $row)
			{
				...
				for($count = 1; $count <= 4; $count++)
				{
					$exam->query = "SELECT option_title FROM option_table 
									WHERE question_id = '".$_POST["question_id"]."' 
	...
	if($_POST['page'] == 'user')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			{
				$sub_array = array();
				$sub_array[] = '<img src="../upload/'.$row["user_image"].'" class="img-thumbnail" width="50" />';
				...
				$action='<input type="button" id="'.$row['user_id'].'" class="btn btn-info fa fa-info-circle details">';
				// $sub_array[] = '';
				$sub_array[]=$action;
				$data[] = $sub_array;
			}
