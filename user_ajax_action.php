...
if(isset($_POST['page']))
{
	...
	if($_POST['page'] == 'index')
	{
		...
		{
			$exam->query = "SELECT * FROM online_exam_table WHERE online_exam_id = '".$_POST['exam_id']."'";
			$result = $exam->query_result();
			...
			foreach($result as $row)
			{
				...
				<tr>
					...
					<td>'.date('d.m.Y',strtotime($row["online_exam_datetime"])).'</td>
				</tr>
				<tr>
		...
		if($_POST['action'] == 'enroll_exam')
		{
			...
			$exam->query = " INSERT INTO user_exam_enroll_table (user_id, exam_id) VALUES (:user_id, :exam_id) ";
			$exam->execute_query();

			$exam->query = "SELECT question_id FROM question_table WHERE online_exam_id = '".$_POST['exam_id']."'";
			$result = $exam->query_result();
			foreach($result as $row)
			{
				$exam->data = array(
				...
				$exam->query = "INSERT INTO user_exam_question_answer 
	...
	if($_POST["page"] == 'enroll_exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			$exam->query = "SELECT * FROM user_exam_enroll_table INNER JOIN online_exam_table 
		...
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				...
				if($next_id == "")
				{
					...
					$buttonType='hidden';
					$endTime='<a href="enroll_exam.php" class="btn btn-danger btn-lg">Sınavı kaydet ve bitir</a>';
				}else{
					$buttonType='button';
				}
				...
				$output .= '
					<br /><br />
				  	<div align="center">
				   		...
				   		<input type="'.$buttonType.'" name="next" class="btn btn-warning btn-lg next" id="'.$next_id.'" value="Next">
				   		'.@$endTime.'
				  	</div>
				  	<br /><br />';
			}
